...
class TestIRToCanvas(unittest.TestCase):
    ...
    def test_break_node(self):
        ...
        ir.add_node(IRNode(
            id="0",
            schema_id="builtin.break",
            kind=NodeKind.CUSTOM,
            params={"break_level": IRParam("break_level", 2, "int")},
        ))
        converter = IRToCanvas()
        data, _ = converter.convert(ir)
        self.assertEqual(data["nodes"][0]["display_name"], "Break")
        self.assertEqual(data["nodes"][0]["node_type"], "break")
        self.assertEqual(data["nodes"][0]["break_level"], "2")
    ...
    def test_script_box_node(self):
        ...
        ir.add_node(IRNode(
            ...
            kind=NodeKind.OPAQUE,
            params={
                "code": IRParam("code", "", "string"),
                "script_ref": IRParam("script_ref", "script_0", "string"),
                "script_io_spec": IRParam("script_io_spec", {"inputs": [], "outputs": []}, "dict"),
            },
        ))
        converter = IRToCanvas()
        data, _ = converter.convert(ir)
        self.assertEqual(data["nodes"][0]["display_name"], "Script Box")
        self.assertEqual(data["nodes"][0]["node_type"], "opaque_code")
        self.assertEqual(data["nodes"][0]["script_ref"], "script_0")
    ...
    def test_state_event_shell_node(self):
        ...
        ir.add_node(IRNode(
            ...
            kind=NodeKind.OPAQUE,
            params={
                "shell_name": IRParam("shell_name", "OnEvent", "string"),
                "state_event_fields": IRParam("state_event_fields", {"event_name": "evt"}, "dict"),
            },
        ))
        converter = IRToCanvas()
        data, _ = converter.convert(ir)
        self.assertEqual(data["nodes"][0]["display_name"], "OnEvent")
        self.assertEqual(data["nodes"][0]["external_kind"], "state_event")
