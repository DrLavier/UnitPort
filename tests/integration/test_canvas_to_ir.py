...
from compiler.ir.workflow_ir import NodeKind, EdgeType
...
class TestCanvasToIR(unittest.TestCase):
    ...
    def test_break_node(self):
        data = {
            "nodes": [{
                "id": 0, "display_name": "Break",
                "position": {"x": 100, "y": 100},
                "node_type": "break", "break_level": "2",
            }],
            "connections": [],
        }
        converter = CanvasToIR()
        ir, _ = converter.convert(data, "go2")
        self.assertEqual(ir.nodes[0].schema_id, "builtin.break")
        self.assertEqual(ir.nodes[0].get_param_value("break_level"), 2)

    def test_break_edge_is_flow(self):
        data = {
            "nodes": [
                {"id": 0, "display_name": "Break", "position": {"x": 100, "y": 100},
                 "node_type": "break", "break_level": "1"},
                {"id": 1, "display_name": "Loop", "position": {"x": 400, "y": 100},
                 "node_type": "while_loop", "loop_type": "While", "condition_expr": "x"},
            ],
            "connections": [
                {"from_node": 0, "from_port": "break_out", "to_node": 1, "to_port": "loop_break_in"},
            ],
        }
        converter = CanvasToIR()
        ir, _ = converter.convert(data, "go2")
        self.assertEqual(len(ir.edges), 1)
        self.assertEqual(ir.edges[0].edge_type, EdgeType.FLOW)

    def test_script_box_node(self):
        data = {
            "nodes": [{
                "id": 0, "display_name": "Script Box",
                "position": {"x": 100, "y": 100},
                "node_type": "opaque_code",
                "script_ref": "script_0",
                "script_io_spec": {"inputs": [], "outputs": []},
                "code": "",
            }],
            "connections": [],
        }
        converter = CanvasToIR()
        ir, _ = converter.convert(data, "go2")
        self.assertEqual(ir.nodes[0].schema_id, "builtin.opaque_code")
        self.assertEqual(ir.nodes[0].kind, NodeKind.OPAQUE)

    def test_state_event_shell_node(self):
        data = {
            "nodes": [{
                "id": 0, "display_name": "OnEvent",
                "position": {"x": 100, "y": 100},
                "node_type": "opaque_code",
                "external_kind": "state_event",
                "shell_name": "OnEvent",
                "state_event_fields": {"event_name": "start_evt"},
            }],
            "connections": [],
        }
        converter = CanvasToIR()
        ir, _ = converter.convert(data, "go2")
        self.assertEqual(ir.nodes[0].schema_id, "builtin.opaque_code")
        self.assertEqual(ir.nodes[0].get_param_value("shell_name"), "OnEvent")
