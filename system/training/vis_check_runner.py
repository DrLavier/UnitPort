...
if TYPE_CHECKING:
    from system.training.training_config import VisCheckConfig
...
def _resolve_export_review_command(bundle, spec, log_fn: Optional[LogFn] = None) -> np.ndarray:
    ...
    try:
        from system.training.unitree_gym_env import resolve_task_commands
...
def run_export_bundle_review(
    ...
    try:
        ...
        if log_fn:
            log_fn(f"[review] Export bundle review failed: {exc}")
...
def run_scene_config_preview(
    ...
    try:
        from system.training.sb3_trainer import get_obs_action_dims
        from system.training.unitree_gym_env import UnitreeGymEnv, _resolve_scene_xml
    except ImportError as exc:
        if log_fn:
