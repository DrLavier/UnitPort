...
def auto_tune_for_hardware(
    ...
    * All ``batch_size`` values target GPU *compute* utilisation only; the
    ...
    # ── n_envs ────────────────────────────────────────────────────────────
    # Only adjust when still at the canvas default (8).
    # The resulting value may be higher (strong machine) OR lower (weak one).
    current_n_envs = int(getattr(env_cfg, "n_envs", _DEFAULT_N_ENVS) or _DEFAULT_N_ENVS)
    if current_n_envs == _DEFAULT_N_ENVS:
        optimal_n = _optimal_n_envs(cpu_count, ram_gb)
        if optimal_n != current_n_envs:
            env_cfg.n_envs = optimal_n
            direction = "up" if optimal_n > current_n_envs else "down"
            changes.append(
                f"n_envs  {current_n_envs} -> {optimal_n}  "
                f"(CPU: {cpu_count} cores, RAM: {ram_gb:.0f} GB, {direction})"
            )
