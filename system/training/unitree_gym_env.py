...
UnitreeGymEnv
    ...
    Observation follows the DEFAULT_COMPONENT_ORDER from system.policy.obs_builder
...
def _find_go2_mjcf() -> Optional[str]:
    ...
    candidates = [
        "brands_sdk/Unitree/unitree_mujoco/unitree_robots/go2/go2.xml",
...
def _find_go2_scene_xml(scene_type: str = "flat") -> Optional[str]:
    ...
    candidates = [
        f"brands_sdk/Unitree/unitree_mujoco/unitree_robots/go2/{filename}",
        str(Path(__file__).parent.parent.parent /
            f"brands_sdk/Unitree/unitree_mujoco/unitree_robots/go2/{filename}"),
    ]
    for path in candidates:
    ...
    if scene_type == "custom":
        ...
        if custom:
            ...
            if os.path.exists(abs_custom):
                return abs_custom
        # custom path invalid 闂?fall through to auto-detect
        scene_type = "flat"
...
def _extract_joint_names_from_model(model, action_dim: int) -> List[str]:
    ...
    return list(GO2_JOINT_NAMES[: max(0, int(action_dim or len(GO2_JOINT_NAMES)))])
...
class UnitreeGymEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        try:
            from system.training.training_config import resolve_control_timing
    ...
    def _apply_init_pose(self, rng: np.random.Generator) -> None:
        ...
        n_joints = min(12, self._model.nq - 7)
        ...
        if target_joints is None:
            target_joints = GO2_DEFAULT_QPOS[:n_joints].copy()
        ...
        # Base height
        height = target_height if target_height is not None else 0.32
        if base_height_override >= 0.0:
    ...
    def _action_to_ctrl(self, action: np.ndarray) -> np.ndarray:
        ...
        if self._action_type == "torque":
            torque_scale = _TORQUE_LIMITS[:n_act]
            return np.asarray(action[:n_act], dtype=np.float32) * torque_scale
        qpos_end = min(self._model.nq, 7 + n_act)
        qvel_end = min(self._model.nv, 6 + n_act)
        joint_pos = np.asarray(self._data.qpos[7:qpos_end], dtype=np.float32)
        joint_vel = np.asarray(self._data.qvel[6:qvel_end], dtype=np.float32)
        if joint_pos.shape[0] < n_act:
            joint_pos = np.pad(joint_pos, (0, n_act - joint_pos.shape[0]))
        if joint_vel.shape[0] < n_act:
            joint_vel = np.pad(joint_vel, (0, n_act - joint_vel.shape[0]))
        if self._action_type == "joint_velocity":
            vel_target = np.asarray(action[:n_act], dtype=np.float32)
            return 8.0 * (vel_target - joint_vel[:n_act])
        pos_target = np.asarray(action[:n_act], dtype=np.float32)
        return 20.0 * (pos_target - joint_pos[:n_act]) - 0.5 * joint_vel[:n_act]
    ...
    def _resolve_reward_weights(self) -> Dict[str, float]:
        ...
        if next_idx < n_stages and self._blend_start_step < 0:
            ...
            min_step_ok = self._global_step >= int(getattr(next_stage, "min_step", 0) or 0)

            # Rolling episode mean vs best-ever stability guard
            ...
            if min_step_ok and (stability_ok or hard_timeout):
    ...
    def _compute_reward(
        ...
        if jpt_weight != 0.0:
            ...
            if self._ref_frames is not None:
                ...
                n_def = min(self._act_dim, len(GO2_DEFAULT_QPOS))
                q_ref = GO2_DEFAULT_QPOS[:n_def]
            n = min(len(cur_joints), len(q_ref))
            sq_err = float(np.sum((cur_joints[:n] - q_ref[:n]) ** 2))
            reward += jpt_weight * float(np.exp(-5.0 * sq_err))
        ...
        if jvt_weight != 0.0:
            ...
            reward += jvt_weight * float(np.exp(-0.5 * sq_err))
            ...
            if foot_ref_frame is not None:
                ...
                for fi, bid in enumerate(self._foot_probe_body_ids):
                    ...
                    try:
                        ...
                        pass
                if sq_errs:
                    reward += fpt_weight * float(np.exp(-10.0 * float(np.mean(sq_errs))))
