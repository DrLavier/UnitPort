...
export_bundle(model, spec, run_id, output_root) -> Path
    ...
    6. Atomically moves the staged bundle to
       ``<output_root>/custom_checkpoints/<policy_id_out>/``.
...
class ExportResult:
    ...
    """
    runtime_bundle_path: Optional[Path] = None  # custom_checkpoints/<id>/
...
# ---------------------------------------------------------------------------
# Go2 defaults — used when spec fields are absent
# ---------------------------------------------------------------------------
...
def build_manifest(
    ...
    num_joints = _NUM_JOINTS_BY_ROBOT.get(robot_type, action_dim)
    joint_names = _GO2_JOINT_NAMES[:num_joints]
...
def export_bundle(
    ...
    ``"runtime_bundle"`` (default)
        ONNX/JIT + manifest.yaml → ``<output_root>/custom_checkpoints/<id>/``
        Listed by CheckpointRegistry; usable by the main canvas.
    ``"training_artifact"``
        SB3 zip → ``<output_root>/training_assets/<id>/best/best_model.zip``
    ...
    """
    from system.training.sb3_trainer import get_obs_action_dims
    from system.policy.manifest_schema import validate_manifest
    ...
    if want_runtime:
        ...
        try:
            ...
            checkpoints_dir = output_root / "custom_checkpoints"
    ...
    # ── Training artifact ─────────────────────────────────────────────
    if want_artifact:
        artifacts_dir = output_root / "training_assets"
