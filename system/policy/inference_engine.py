...
class ONNXEngine(InferenceEngine):
    ...
    def __init__(self, providers: Optional[List[str]] = None):
        self._providers: List[str] = providers if providers is not None else list(self._DEFAULT_PROVIDERS)
...
class JITEngine(InferenceEngine):
    ...
    def load(self, model_path: Path) -> None:
        ...
        RuntimeError
            If the file exists but cannot be loaded as a TorchScript module
            (e.g. it is a plain state-dict or corrupt file).
        """
    ...
    def predict(self, obs: np.ndarray) -> np.ndarray:
        ...
        ------
        RuntimeError
            If load() has not been called yet, or if the model output is not a
            Tensor (shape mismatch / unexpected output type).
        """
