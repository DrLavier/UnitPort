...
Checkpoints live under  <project_root>/custom_checkpoints/  (one sub-directory
...
class CheckpointEntry:
    ...
    policy_id: str
    """Directory name under custom_checkpoints/ — used as the canonical ID."""
...
class CheckpointRegistry:
    """
    Scans and indexes checkpoint bundles under ``custom_checkpoints/``.

    Canonical checkpoint storage path:
        <project_root>/custom_checkpoints/<policy_id>/manifest.yaml
    ...
    STORAGE_SUBDIR = "custom_checkpoints"
    ...
    def root(self) -> Path:
        ...
        try:
            from bin.core.config_manager import ConfigManager  # lazy import
            return ConfigManager().get_path("project_root") / self.STORAGE_SUBDIR
        except Exception:
    ...
    def discover(self) -> List[CheckpointEntry]:
        ...
        for candidate in sorted(self.root.iterdir()):
            ...
            if not manifest_path.exists():
                continue
            if not self._is_runtime_bundle(candidate):
                continue
    ...
    @staticmethod
    def _is_runtime_bundle(bundle_path: Path) -> bool:
        """Return True only for runnable runtime bundles."""
        try:
            from system.policy.bundle_loader import BundleLoader
            BundleLoader().load(bundle_path)
            return True
        except Exception:
            return False
