...
from bin.components.sidebar_dock import SidebarDock
from bin.core.config_manager import ConfigManager
from bin.core.logger import log_error
from bin.core.theme_manager import get_color_slot
from system.training.robot_family import resolve_robot_family
from system.training.task_template_resolver import resolve_task_template
...
class TrainingStatsPopup(QFrame):
    ...
    def _build_ui(self) -> None:
        ...
        close_icon, has_icon = _load_icon("assets/icon/ICON_CL_W.svg")
        if has_icon:
    ...
    def _build_run_entries(self) -> list:
        ...
        window = self._anchor_bar.window()
        workspace_policy = str(getattr(window, "_workspace_policy_id", "") or getattr(window, "_policy_id", "") or "").strip()
        active_run_id = str(getattr(window, "_active_run_id", "") or "").strip()

        filtered = []
        for table in tables:
            if workspace_policy and table.get("policy_id") and table.get("policy_id") != workspace_policy:
                continue
    ...
    def _rebuild_series(self) -> None:
        ...
        for run_id in sorted(self._selected_runs):
            ...
            for field_name, display_name in metric_defs:
                ...
                color_idx += 1
                points = [
                    (float(sample.get("step", 0) or 0), float(sample.get(field_name, 0.0) or 0.0))
...
class TrainingFloatControlBar(QWidget):
    ...
    def _apply_icons(self):
        _bindings = [
            (self._drag_btn,  "assets/icon/icon_move.svg",  ""),
            (self._start_btn, "assets/icon/icon_play.svg",  ">"),
            (self._stop_btn,  "assets/icon/icon_stop.svg",  "[]"),
            (self._stats_btn, "assets/icon/icon_graphic.svg", ""),
        ]
        for btn, path, fallback in _bindings:
            icon, ok = _load_icon(path, fallback)
            if ok:
                btn.setIcon(icon)
                btn.setText("")
            else:
...
def _get_workspace_store():
    ...
    from system.training.training_workspace_store import TrainingWorkspaceStore
...
class TrainingGraphScene(GraphScene):
    ...
    def _create_logic_node(self, name: str, node_id: int, rect_item):
        ...
        try:
            from nodes.sys_nodes.training_nodes import (
                RobotMJCFNode,
                PhysicsConfigNode,
                RewardsNode,
                TerminationsNode,
    ...
    def create_node(self, name: str, scene_pos: QPointF, **kwargs):
        ...
        from bin.components.training_node_items import TrainingNodeItem
    ...
    def _apply_port_visual(self, port_item, state: str = "normal"):
        ...
        """
        from bin.components.training_node_items import TrainingNodePort
    ...
    def _create_connection(self, out_port, in_port):
        ...
        try:
            ...
            for old_conn in existing_in:
                try:
                    from bin.components.graph_scene import isValid
...
class TrainingCanvasWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Floating control bar 鈥?child of the view so it overlays the canvas
...
class TrainingWorkspaceWindow(QWidget):
    ...
    def _build_body(self) -> QWidget:
        ...
        self._sidebar = SidebarDock(
            nav_items=[
                ("canvas", "Experiments", "icon_prj_dark", "assets/icon/icon_prj_w.svg"),
                ("exports", "Exports", "icon_cp_dark", "assets/icon/icon_cp_w.svg"),
                ("nodes", "Config Nodes", "icon_nod_dark", "assets/icon/icon_nod_w.svg"),
            ],
            panel_width=320,
        )
    ...
    def _load_checkpoint_source_info(self, policy_id: str = "") -> dict:
        try:
            ...
            if not resolved_policy_id:
                return {}
            source_path = get_project_root() / "custom_checkpoints" / resolved_policy_id / "source.json"
    ...
    def _match_contract_preset(entry) -> str:
        from system.training.obs_contracts import get_obs_contract, list_preset_names
    ...
    def start_training(self, spec) -> None:
        ...
        import time
        import uuid
        from bin.core.train_run_thread import TrainRunThread
        ...
        if self._ws_store is not None:
            try:
                from system.training.training_workspace_store import RunMeta
    ...
    def _on_train_requested(self, _job_spec: dict) -> None:
        ...
        """
        try:
            from system.training.training_spec import TrainingSpecCompiler
            ...
            # 3-C: resolve BaseAssetNode 鈫?absolute checkpoint path
        ...
        # Use intended_mode (user's canvas intent) rather than spec.resume_mode
        # so that a failed asset resolution also triggers a block 鈥?not a silent
        # fallback to scratch.
        if intended_mode != "scratch":
            if entry is None:
                self._strip_state.setText("Asset Error")
                self._strip_label.setText(
                    "Base asset could not be resolved 鈥?cannot resume/warm-start."
                )
                return
    ...
    def _on_review_requested(self, _job_spec: dict) -> None:
        ...
        from system.training.vis_check_runner import run_environment_review
    ...
    def _on_export_review_requested(self, _job_spec: dict) -> None:
        ...
        from system.training.vis_check_runner import run_export_bundle_review
        self._run_viewer_task(
            "Export Review",
    ...
    def _on_scene_preview_requested(self, _scene_params: dict) -> None:
        ...
        from system.training.vis_check_runner import run_scene_config_preview
        self._run_viewer_task(
            "Preview",
    ...
    def _on_init_pose_preview_requested(self, _params: dict) -> None:
        ...
        from system.training.vis_check_runner import run_init_pose_preview
        self._run_viewer_task(
            "Init Pose Preview",
    ...
    def _append_training_log(self, message: str) -> None:
        ...
        if panel is not None:
            panel.on_log(msg)
            return
        from bin.core.logger import log_info
        log_info(msg)
    ...
    def _resolve_export_review_bundle_path(self, spec):
        ...
        candidate = get_project_root() / "custom_checkpoints" / str(policy_id_out)
        if candidate.exists():
            return candidate
        raise FileNotFoundError(
    ...
    def _run_compat_check(self, entry, spec) -> bool:
        ...
        """
        try:
            from system.training.training_compatibility import (
                CompatLevel,
    ...
    def _init_workspace(self) -> None:
        ...
        try:
            ...
            if active_id and active_id in [e.experiment_id for e in meta.experiments]:
                self._load_experiment_by_id(active_id)
    ...
    def _import_training_asset_local(self) -> None:
        from pathlib import Path
        from system.training.training_asset_registry import TrainingAssetImportError
    ...
    def _download_training_asset_hf(self) -> None:
        from bin.core.hf_training_asset_download_thread import HFTrainingAssetDownloadThread
    ...
    def _load_experiment_by_id(self, experiment_id: str) -> None:
        ...
        try:
            ...
            self._strip_label.setText(str(exc)[:80])
            QMessageBox.warning(self, "Load Experiment Failed", str(exc))
