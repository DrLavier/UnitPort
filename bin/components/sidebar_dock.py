...
class SidebarDock(QWidget):
    ...
    def __init__(
        ...
        for key, title, icon_key, fallback_icon in items:
            ...
            self._nav_icon_bindings[key] = (icon_key, fallback_icon)
    ...
    def refresh_icons(self, theme: str):
        ...
        for key, button in self._nav_buttons.items():
            base_key, fallback_icon = self._nav_icon_bindings.get(key, ("", ""))
    ...
    def _resolve_icon_path(self, key: str, fallback_rel_path: str) -> Path:
        ...
        if self.config is not None:
            rel_or_abs = str(
                self.config.get("UI", key, fallback=fallback_rel_path, config_type="system") or ""
            ).strip() or fallback_rel_path
        icon_path = Path(rel_or_abs)
        if not icon_path.is_absolute() and self.config is not None:
            icon_path = self.config.project_root / icon_path
        return icon_path

    def _apply_button_icon(self, button: QPushButton, key: str, fallback_rel_path: str):
        icon_path = self._resolve_icon_path(key, fallback_rel_path)
        if icon_path.exists():
    ...
    def _dark_icon_fallback(self, light_fallback: str) -> str:
        stem = Path(light_fallback)
        if stem.suffix.lower() != ".svg":
            return light_fallback
        return str(stem.with_name(f"{stem.stem}_w{stem.suffix}"))
