...
from bin.core.theme_manager import get_color, get_font_size, get_color_slot

# ---------------------------------------------------------------------------
# Asset paths
# ---------------------------------------------------------------------------
_ROOT     = Path(__file__).parent.parent.parent
_ICON_DIR = _ROOT / "assets" / "icon"
...
class WindowControlButtons(QWidget):
    ...
    def _wc_icon(self, dark_name: str, light_name: str) -> QIcon:
        preferred_name = light_name if self._theme == "light" else dark_name
        fallback_name = dark_name if self._theme == "light" else light_name
        preferred_path = _ICON_DIR / preferred_name
        fallback_path = _ICON_DIR / fallback_name
        path = preferred_path
        if not self._svg_has_visible_content(preferred_path) and self._svg_has_visible_content(fallback_path):
            path = fallback_path
        return QIcon(str(path))

    def _refresh_button_icons(self) -> None:
        icon_size = self._icon_size()
        self.minimize_button.setIcon(self._wc_icon("ICON_MIN_W.svg", "ICON_MIN.svg"))
        self.minimize_button.setIconSize(icon_size)
        fs_dark  = "ICON_WW_W.svg" if self._is_fullscreen else "ICON_FW_W.svg"
        fs_light = "ICON_WW.svg"   if self._is_fullscreen else "ICON_FW.svg"
        self.fullscreen_button.setIcon(self._wc_icon(fs_dark, fs_light))
        self.fullscreen_button.setIconSize(icon_size)
        self.close_button.setIcon(self._wc_icon("ICON_CL_W.svg", "ICON_CL.svg"))
...
def _svg_logo(theme: str, width: int = 800) -> _SvgLogo:
    filename = "START_LOGO.svg" if theme == "light" else "START_LOGO_W.svg"
    path = _ICON_DIR / filename
    if not path.exists():
        alt  = "START_LOGO_W.svg" if theme == "light" else "START_LOGO.svg"
        path = _ICON_DIR / alt
    return _SvgLogo(str(path), logo_width=width)
...
class LoadingScreen(_GradientBase):
    ...
    def _setup_ui(self):
        ...
        logo = QSvgWidget(str(_ROOT / "assets" / "icon" / "logo_w.svg"), self)
