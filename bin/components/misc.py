...
class MainRow(QWidget):
    """Reusable top navigation row preserving the original layout behavior."""
    ...
    def __init__(self, theme: str = "dark", parent=None):
        ...
        self._left_mirror = QWidget(self)
        self._left_mirror.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        self._left_mirror.setStyleSheet("background: transparent; border: none;")
        self._left_mirror.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Preferred)
        self._left_mirror.setFixedWidth(0)
        self._layout.addWidget(self._left_mirror, 0)

        self._layout.addStretch(1)

        self._page_switcher = PageSwitcher(self)
        self._page_switcher.page_selected.connect(self.page_selected.emit)
        self._page_switcher.page_mode_changed.connect(self._on_page_mode_changed)
        self._layout.addWidget(self._page_switcher, 0, Qt.AlignmentFlag.AlignVCenter)
        ...
        # Preferred (not Fixed): let Qt size from content so early sizeHint() misses don't clip
    ...
    def _on_page_mode_changed(self, page: str) -> None:
        self._current_page = page
        self.apply_page_bg()
    ...
    def sync_layout(self) -> None:
        # Use the actual laid-out width when available; sizeHint only as a cold-start estimate.
        right_width = self._right_host.width()
        if right_width == 0 and self._right_widget is not None:
            self._right_widget.adjustSize()
            right_width = max(
                self._right_widget.sizeHint().width(),
                self._right_widget.minimumSizeHint().width(),
            )
            # Also probe a QStackedWidget's current page if present
            current_widget = getattr(self._right_widget, "currentWidget", lambda: None)()
            if current_widget is not None:
                current_widget.adjustSize()
                right_width = max(
                    right_width,
                    current_widget.sizeHint().width(),
                    current_widget.minimumSizeHint().width(),
                )
        # Mirror the left side to keep PageSwitcher centered — _right_host sizes itself naturally.
        if self._left_mirror.width() != right_width:
            self._left_mirror.setFixedWidth(right_width)
            self.updateGeometry()
