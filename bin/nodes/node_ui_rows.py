...
class InputSetting(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        self._key_bg_color = key_bg_color
        self._input_bg_color = input_bg_color
        self._border_color = border_color
        self._text_color = text_color
        self._show_left_dot = bool(show_left_dot)
        self._show_right_dot = bool(show_right_dot)
        self._externally_bound = False
        self._saved_value = ""
    ...
    def set_external_binding(self, text: str) -> None:
        if not self._externally_bound:
            self._saved_value = self.input_edit.text()
        self._externally_bound = True
        self.input_edit.setText(text or "")
        self.input_edit.setEnabled(False)
        self._refresh_style()

    def clear_external_binding(self) -> None:
        if not self._externally_bound:
            return
        self._externally_bound = False
        self.input_edit.setEnabled(True)
        self.input_edit.setText(self._saved_value)
        self._saved_value = ""
        self._refresh_style()

    def is_externally_bound(self) -> bool:
        return self._externally_bound
    ...
    def _refresh_style(self) -> None:
        border_color = _muted_color(self._border_color, 0.42) if self._externally_bound else self._border_color
        dot_bg = _muted_color(self._dot_bg_color, 0.3) if self._externally_bound else self._dot_bg_color
        key_bg = _muted_color(self._key_bg_color, 0.2) if self._externally_bound else self._key_bg_color
        input_bg = _muted_color(self._input_bg_color, 0.2) if self._externally_bound else self._input_bg_color
        text_color = _muted_color(self._text_color, 0.45) if self._externally_bound else self._text_color
        key_left_border = "0px" if self._show_left_dot else f"1px solid {border_color}"
        self.key_box.setStyleSheet(
            f"QLabel {{ background: {key_bg}; color: {text_color}; "
            f"border: 1px solid {border_color}; border-left: {key_left_border}; "
            f"border-radius: 0px; padding: 2px 6px; font-size: 10px; }}"
        )
        self.input_edit.setStyleSheet(
            f"QLineEdit {{ background: {input_bg}; color: {text_color}; "
            f"border: 1px solid {border_color}; border-left: 0px; "
            f"border-radius: 0px; padding: 2px 6px; font-size: 10px; }}"
            f"QLineEdit:disabled {{ background: {input_bg}; color: {text_color}; }}"
        )
        if self.left_box is not None:
            self.left_box.setStyleSheet(
                f"QFrame {{ border: 1px solid {border_color}; "
                f"background: {dot_bg}; border-radius: 0px; }}"
            )
...
class ComboSetting(QWidget):
    ...
    def set_external_binding(self, text: str) -> None:
        value = str(text or "")
        if not self._externally_bound:
            self._saved_value = self.combo.currentText()
        self._externally_bound = True
        if self.combo.isEditable():
            ...
            if self.combo.findText(value) < 0:
                ...
                self._bound_injected_text = value
    ...
    def clear_external_binding(self) -> None:
        if not self._externally_bound:
            return
        injected = self._bound_injected_text
        self._externally_bound = False
        self.combo.setEnabled(True)
        if injected:
            idx = self.combo.findText(injected)
            if idx >= 0:
                self.combo.removeItem(idx)
            self._bound_injected_text = ""
        if self.combo.isEditable():
            self.combo.setEditText(self._saved_value)
        else:
            self.combo.setCurrentText(self._saved_value)
        self._saved_value = ""
        self._refresh_style()
    ...
    def _refresh_style(self) -> None:
        ...
        dot_bg = _muted_color(self._dot_bg_color, 0.3) if self._externally_bound else self._dot_bg_color
        key_bg = _muted_color(self._key_bg_color, 0.2) if self._externally_bound else self._key_bg_color
        combo_bg = _muted_color(self._combo_bg_color, 0.2) if self._externally_bound else self._combo_bg_color
        text_color = _muted_color(self._text_color, 0.45) if self._externally_bound else self._text_color
        key_left_border = "0px" if self._show_left_dot else f"1px solid {border_color}"
        right_border = "0px" if self._show_right_dot else "1px"
        self.key_box.setStyleSheet(
            f"QLabel {{ background: {key_bg}; color: {text_color}; "
            f"border: 1px solid {border_color}; border-left: {key_left_border}; "
            f"border-radius: 0px; padding: 2px 6px; font-size: 10px; }}"
        ...
        self.combo.setStyleSheet(
            f"QComboBox {{ background: {combo_bg}; color: {text_color}; "
            ...
            f"QComboBox:disabled {{ background: {combo_bg}; color: {text_color}; }} "
            f"QComboBox::item {{ color: {text_color}; background: {combo_bg}; }} "
            f"QComboBox::item:selected {{ background: {border_color}; color: {text_color}; }} "
            f"QComboBox QAbstractItemView {{ color: {text_color}; }}"
            f"QComboBox QLineEdit {{ background: {combo_bg}; color: {text_color}; border: 0px; padding: 0px; }}"
        )
        if self.left_box is not None:
            self.left_box.setStyleSheet(
                f"QFrame {{ border: 1px solid {border_color}; "
...
def _muted_color(color: str, toward_white: float) -> str:
    base = QColor(color)
    if not base.isValid():
        return color
    mix = max(0.0, min(1.0, float(toward_white)))
    r = round(base.red() + (255 - base.red()) * mix)
    g = round(base.green() + (255 - base.green()) * mix)
    b = round(base.blue() + (255 - base.blue()) * mix)
