...
## Mission Persistence — Protocol Migration (Step 5 / schema v1.4)

### `mission_persistence.py` additions

| Function | Signature | Purpose |
|----------|-----------|---------|
| `migrate_mission_payload` | `(data) → dict` | Inspect mission dict for protocol upgrade need; **never mutates** |

`migrate_mission_payload` return shape:

```python
{
    "needs_protocol_upgrade": bool,   # True when schema_version < "1.4" or absent
    "prior_schema_version":   str,    # value from file, or "unknown"
    "warnings":               list,   # human-readable notices; empty when up-to-date
}
```

`MISSION_SCHEMA_VERSION` is `"1.4"` as of Step 5.  Files at earlier versions
(including those without a `schema_version` key) are considered pre-protocol
and trigger the migration-compat connection-restore path in `GraphScene`
(see `bin/components/README.md` for details).

`MainWindow._on_open()` calls `migrate_mission_payload()` before loading and
surfaces any warnings via `log_info` so the user is informed without blocking
the open operation.
