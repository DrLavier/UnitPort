...
"""Lookbook widgets for browsing MuJoCo Menagerie packages.

Shared by both the first-launch wizard and the sidebar Robot Asset browser
dialog. Pure widgets — they consume a snapshot of (installed, registered,
pre-checked) sets and emit ``selection_changed`` / ``toggled_changed``;
callers own the lifecycle (fetch live list, kick icon-fetch worker,
trigger sparse-checkout).
...
All colors come from ``system.ini[Theme]`` via ``Config.get_color`` per
CLAUDE.md §1.5 — no hex literals. New theme slots (added with this widget):

    menagerie_card_bg
    menagerie_card_bg_hover
    menagerie_card_bg_installed
    menagerie_card_border_installed
    menagerie_card_bg_checked
    menagerie_card_title_registered
"""
...
def _stylesheet() -> str:
    bg_card = Config.get_color("menagerie_card_bg")
    bg_hover = Config.get_color("menagerie_card_bg_hover")
    bg_main = Config.get_color("bg_2")
    text_main = Config.get_color("main_t2")
    text_muted = Config.get_color("sub_t2")
    border = Config.get_color("border_1")
    accent = Config.get_color("highlight")
    accent_blue = Config.get_color("menagerie_card_title_registered")
    installed_bg = Config.get_color("menagerie_card_bg_installed")
    installed_border = Config.get_color("menagerie_card_border_installed")
    checked_bg = Config.get_color("menagerie_card_bg_checked")
...
__all__ = [
    ...
    "MenagerieCard", "MenagerieCardGrid",
]
