    ...
    def _stage_install_lab(self) -> None:
        ...
        try:
            constraints_file.write_text(
                "# Auto-generated by IsaacLabInstaller.\n"
                "# (1) Cap setuptools so legacy sdists (flatdict==4.0.1, etc.)\n"
                "#     can still import pkg_resources during their build phase.\n"
                "setuptools<81\n"
                "# (2) Pin h5py to the version whose bundled HDF5 matches this\n"
                "#     Isaac Sim release. isaaclab_rl declares a bare 'h5py';\n"
                "#     left unpinned pip pulls h5py 3.16 whose newer bundled\n"
                "#     HDF5 diverges from Isaac Sim's, crashing the sensor DLL\n"
                "#     load at training time (entry point H5Tdecode not found\n"
                "#     in hdf5_cpp.dll — the launcher imports h5py before Kit,\n"
                "#     so its HDF5 wins base-name resolution). PIP_CONSTRAINT is\n"
                "#     authoritative: pip cannot violate it in either resolve\n"
                "#     pass, so the [all] RL extras can't pull a wrong h5py.\n"
                f"{ISAAC_SIM_RELEASE.h5py_pin}\n",
                encoding="utf-8",
            )
        except OSError as exc:
            log_warning(
