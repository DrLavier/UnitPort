...
| `training_motion`    | B    | `TrainingCommandsNode + ReferenceMotionNode` | `spec.motion` + `spec.il.motion_ref` | Single source for command envelope + reference clip — guarantees train=reference=execute alignment. `consumption_mode` picks `amp_discriminator` / `tracking_target` / `hybrid` (only `amp_discriminator` implemented today; the other two are reserved and raise on use). |
