...
Resolution order:
1. /runtime/simulation/mujoco/menagerie
2. /runtime/sdk/<Brand>/...
"""
...
from src.system.models.model_registry import get_model_spec, iter_model_specs
...
_REGISTERED_MODEL_RULES: Dict[Tuple[str, str], MujocoAssetRule] = {
    ...
    ("boston_dynamics", "spot"): MujocoAssetRule("boston_dynamics", "spot", ("boston_dynamics_spot",)),
    # Temporarily unregistered in system.model_registry: no MuJoCo assets available.
...
def _brand_asset_roots(brand: str) -> Iterable[Path]:
    root = _project_root() / "runtime/sdk"
    ...
    if brand_key == "unitree":
        ...
        yield root / "Unitree"
        yield root / "unitree"
    elif brand_key == "boston_dynamics":
        yield root / "boston_dynamics"
        yield root / "boston_dynamics"
        yield root / "boston_dynamics" / "spot-sdk"
...
def _menagerie_candidates(rule: MujocoAssetRule) -> Iterable[Path]:
    root = _project_root() / "runtime" / "simulation" / "mujoco" / "menagerie"
...
def _fallback_candidates(rule: MujocoAssetRule) -> Iterable[Path]:
    ...
    for base in _brand_asset_roots(rule.brand_id):
        ...
        if rule.brand_id == "boston_dynamics":
