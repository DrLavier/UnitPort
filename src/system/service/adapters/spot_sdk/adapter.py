...
def _s_diag(message: str = "", context: dict | None = None, **fields) -> dict:
    ...
    base: dict = {"brand": "boston_dynamics", "message": message, "context": context or {}}
    base.update(fields)
    return base
...
class SpotAdapter(BaseAdapter):
    ...
    def accept_action(
        ...
        """Translate a policy action to Spot SDK command."""
        if not self._session_open:
            return {"ok": False, "reason": _REASON_NO_SESSION}
        # Spot policy actions routed through command client
    ...
    def check_precondition(self, precondition: Dict[str, Any]) -> bool:
        """Check if Spot satisfies the skill precondition."""
        posture = precondition.get("posture", "any")
        if posture == "any":
            return True
        if not self._session_open:
            return False
        # E-stop must be clear for any posture
        return True
