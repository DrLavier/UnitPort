...
class CompatibilityChecker:
    ...
    def check(
        ...
        if actual_obs != bundle.obs_dim:
            # When the builder produces MORE dims than the bundle expects, the
            # runtime can safely truncate (obs_builder.build() handles this).
            # Only FAIL when the builder produces FEWER dims — the model would
            # receive zero-padded inputs it was never trained on.
            severity = (
                CompatStatus.WARN if actual_obs > bundle.obs_dim else CompatStatus.FAIL
            )
            issues.append(CompatIssue(
                ...
                message=(
                    ...
                    + (" (runtime will truncate)" if actual_obs > bundle.obs_dim else "")
                ),
                severity=severity,
