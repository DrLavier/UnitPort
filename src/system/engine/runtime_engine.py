...
from src.system.ir.workflow_ir import NodeKind, WorkflowIR
...
class RuntimeEngine:
    ...
    def execute(self, mission_ir: Any, scenario: Any, node_status_callback=None) -> dict:
        ...
        # Phase 5 Task A4: DAG pre-flight transition validation.
        # Extract SkillManifests from the mission IR and validate the chain.
        # Three outcomes: valid (proceed), needs_insertion (auto-insert
        # transition nodes), or blocked (incompatible, cannot execute).
        transition_report = self._run_transition_preflight(mission_ir)

        if transition_report is not None and not transition_report.is_valid:
            self._audit_logger.record("transition_blocked", {
                "incompatible_count": sum(
                    1 for r in transition_report.results
                    if r.status.value == "incompatible"
                ),
            })
            return RuntimeResult.blocked(
                path=ExecutionPath.BLOCKED_COMPILE,
        ...
        try:
            ...
            if self._is_execution_graph(mission_ir):
                ...
                from src.system.engine.behavior_invoker import BehaviorSubgraphInvoker
            ...
            # Phase 5 Task A4: attach transition validation report to diagnostics.
            if transition_report is not None:
                result.diagnostics[DiagnosticsKey.TRANSITION_VALIDATION_REPORT] = [
                    {
                        ...
                        "warnings": [
                            {"kind": w.kind.value, "message": w.message}
                            for w in r.warnings
                        ],
                    }
                    for r in transition_report.results
                ]
                result.diagnostics[DiagnosticsKey.TRANSITION_INSERTED_NODES] = (
                    transition_report.insertions
                )
                if transition_report.insertions:
                    result.diagnostics[DiagnosticsKey.TRANSITION_VALIDATION_STATUS] = "needs_insertion"
                else:
                    result.diagnostics[DiagnosticsKey.TRANSITION_VALIDATION_STATUS] = "valid"
            else:
                result.diagnostics[DiagnosticsKey.TRANSITION_VALIDATION_STATUS] = "valid"
                result.diagnostics[DiagnosticsKey.TRANSITION_VALIDATION_REPORT] = []
                result.diagnostics[DiagnosticsKey.TRANSITION_INSERTED_NODES] = []
    ...
    @staticmethod
    def _run_transition_preflight(mission_ir: Any):
        """Extract SkillManifests from mission_ir and validate the transition chain.

        Returns a TransitionValidationReport, or None when skill manifests
        cannot be extracted (e.g. legacy mission without skill metadata).
        """
        try:
            ...
            if isinstance(mission_ir, WorkflowIR):
                for node in mission_ir.nodes:
                    sm = getattr(node, "skill_manifest", None)
                    if sm is None:
                        node_data = node.to_dict() if hasattr(node, "to_dict") else {}
                        sm = node_data.get("skill_manifest")
                    if isinstance(sm, SkillManifest):
                        manifests.append(sm)
            elif isinstance(mission_ir, dict):
            ...
            return validate_skill_chain(manifests)
        except Exception:
            return None
