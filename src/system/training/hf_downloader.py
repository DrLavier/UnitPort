...
from src.system.policy.manifest_schema import ManifestValidationError, validate_manifest
...
def _build_runtime_manifest(snapshot_dir: Path, repo_id: str) -> dict:
    ...
    # --- SkillManifest v2: infer "skill" section from ONNX shapes + heuristics ---
    try:
        from src.system.skill.manifest_inferrer import infer_skill_manifest_fields

        onnx_path = None
        if policy_format == "onnx":
            onnx_path = snapshot_dir / policy_file

        skill_fields = infer_skill_manifest_fields(
            snapshot_dir,
            onnx_path=onnx_path,
            repo_id=repo_id,
        ...
        # Back-fill v1 required fields from skill inference when missing
        manifest.setdefault("observation_space", {}).setdefault(
            "dim", skill_fields.get("observation_dim", 45),
        )
        act = manifest.setdefault("action_space", {})
        act.setdefault("dim", skill_fields.get("action_dim", 12))
        act.setdefault("type", skill_fields.get("action_space_type", "joint_position"))

        rt = manifest.setdefault("runtime", {})
        rt.setdefault("control_frequency_hz", skill_fields.get("control_frequency_hz", 50.0))
        rt.setdefault("decimation", 4)

        robot = manifest.setdefault("robot", {})
        robot.setdefault("brand", "unknown")
        num_joints = skill_fields.get("action_dim", 12)
        robot.setdefault("num_joints", num_joints)
        robot.setdefault("joint_names", [f"joint_{i}" for i in range(num_joints)])
    except Exception:  # noqa: BLE001
        # SkillManifest inference is best-effort; v1 manifest is sufficient
        pass
