...
MetricsFn   = Callable[[Dict], None]      # fn(metrics_dict) — 4-layer diagnostic metrics
...
class SB3ProgressCallback:
    ...
    def __init__(
        ...
        try:
            ...
            # Layer classification for reward component metrics
            _BEHAVIOR_KEYS = frozenset({
                "velocity", "yaw", "reference_tracking",
                "joint_pose_tracking", "joint_vel_tracking",
                "foot_pos_tracking", "foot_air_time",
            })
            _STABILITY_KEYS = frozenset({
                "upright", "alive", "base_height", "base_height_err", "angular_rate",
            })
            _ACTION_KEYS = frozenset({
                "energy", "smoothness", "slip", "foot_clearance", "collision",
            })
            _ALGO_LOGGER_KEYS = (
            ...
            class _Inner(BaseCallback):
                def __init__(inner_self, **kwargs):
                    ...
                    inner_self._metrics_fn = metrics_fn
                    ...
                    # Component accumulation (reset each rollout)
                    inner_self._comp_accum: Dict[str, float] = {}
                    inner_self._comp_count: int = 0
                ...
                def _on_step(inner_self) -> bool:
                    ...
                    if inner_self._cancel_fn and inner_self._cancel_fn():
                        ...
                        return False
                    # Accumulate reward components from env infos (vectorised env → list)
                    if inner_self._metrics_fn:
                        infos = inner_self.locals.get("infos") or []
                        for info in infos:
                            comps = info.get("reward_components")
                            if isinstance(comps, dict):
                                for k, v in comps.items():
                                    inner_self._comp_accum[k] = (
                                        inner_self._comp_accum.get(k, 0.0) + float(v)
                                    )
                                inner_self._comp_count += 1
                    return True
                ...
                def _on_rollout_end(inner_self) -> None:
                    ...
                    # ── 4-layer metrics ──────────────────────────────────
                    if inner_self._metrics_fn:
                        # Reward component means for this rollout
                        n = inner_self._comp_count or 1
                        means = {k: v / n for k, v in inner_self._comp_accum.items()}
                        inner_self._comp_accum = {}
                        inner_self._comp_count = 0

                        behavior  = {k: v for k, v in means.items() if k in _BEHAVIOR_KEYS}
                        stability = {k: v for k, v in means.items() if k in _STABILITY_KEYS}
                        action_q  = {k: v for k, v in means.items() if k in _ACTION_KEYS}

                        # Algorithm internals from SB3's internal logger
                        algo_metrics: Dict[str, float] = {}
                        try:
                            name_to_val = inner_self.model.logger.name_to_value
                            for key in _ALGO_LOGGER_KEYS:
                                if key in name_to_val:
                                    short = key.split("/", 1)[-1]
                                    algo_metrics[short] = float(name_to_val[key])
                        except Exception:
                            pass

                        inner_self._metrics_fn({
                            "behavior":     behavior,
                            "stability":    stability,
...
def get_obs_action_dims(spec: "TrainingJobSpec") -> Tuple[int, int]:
    ...
    action_dim = (
        ...
        else _ACTION_DIM_BY_ROBOT.get(robot_type, 12)
    )
    try:
        from src.system.training.unitree_gym_env import resolve_obs_dim_from_components
            ...
            class _Inner(BaseCallback):
                ...
                def _on_rollout_end(inner_self) -> None:
                    ...
                    try:
                        ...
                        # Save VecNormalize stats alongside weights so rollback
                        # can restore the observation distribution faithfully.
                        try:
                            from stable_baselines3.common.vec_env import VecNormalize as _VN
                            vec_env = inner_self.training_env
                            if isinstance(vec_env, _VN):
                                vec_env.save(str(_save / "best_model_vecnormalize.pkl"))
                        except Exception:
                            pass
...
class SB3CollapseGuard:
    ...
    def __init__(
        ...
        try:
            ...
            _best_vn     = _best_dir / "best_model_vecnormalize.pkl"
            _max_rollbacks = max(0, int(max_rollbacks))
            ...
            class _Inner(BaseCallback):
                def __init__(inner_self):
                    ...
                    inner_self._orig_learning_starts: int = 0
                    inner_self._cooldown_until_step: int = 0
                ...
                def _rollback(inner_self) -> bool:
                    """Restore best checkpoint. Returns False when max_rollbacks hit."""
                    if not _best_zip.exists():
                        if _log:
                            _log("[guard] No best_model.zip yet — skipping rollback")
                        inner_self._bad_streak = 0
                        return True
                    # Hard stop if max_rollbacks exceeded
                    if _max_rollbacks > 0 and inner_self._rollback_count >= _max_rollbacks:
                        if _log:
                            _log(
                                f"[guard] max_rollbacks={_max_rollbacks} reached — "
                                "stopping training to prevent oscillation."
                            )
                        return False
                    try:
                        ...
                        del snap
                        # Restore VecNormalize running stats — without this, the
                        # restored weights operate on a drifted obs distribution.
                        try:
                            from stable_baselines3.common.vec_env import VecNormalize as _VN
                            vec_env = inner_self.training_env
                            if isinstance(vec_env, _VN) and _best_vn.exists():
                                saved_vn = _VN.load(str(_best_vn), venv=vec_env.venv)
                                vec_env.obs_rms     = saved_vn.obs_rms
                                vec_env.ret_rms     = saved_vn.ret_rms
                                vec_env.clip_obs    = saved_vn.clip_obs
                                vec_env.clip_reward = saved_vn.clip_reward
                                del saved_vn
                                if _log:
                                    _log("[guard] VecNormalize stats restored from snapshot.")
                        except Exception as _vn_exc:
                            if _log:
                                _log(f"[guard] VecNormalize restore skipped: {_vn_exc}")
                        # Flush the replay buffer — without this, SAC continues
                        # learning from thousands of post-collapse garbage transitions
                        # even after weights and normalisation are restored.
                        # After flushing, push learning_starts forward so SB3
                        # refills the buffer before attempting gradient updates
                        # (prevents ValueError: high <= 0 from empty buffer).
                        try:
                            rb = getattr(inner_self.model, "replay_buffer", None)
                            if rb is not None:
                                rb.pos  = 0
                                rb.full = False
                                orig_ls = inner_self._orig_learning_starts
                                refill = max(orig_ls, inner_self.model.batch_size * 2)
                                inner_self.model.learning_starts = (
                                    inner_self.model.num_timesteps + refill)
                                if _log:
                                    _log(f"[guard] Replay buffer flushed; "
                                         f"gradient updates paused for {refill:,} steps")
                        except Exception:
                            pass

                        # Clear episode info buffer — without this the
                        # stale collapsed rewards immediately re-trigger
                        # collapse detection after rollback.
                        try:
                            ep_buf = getattr(inner_self.model, "ep_info_buffer", None)
                        ...
                        inner_self._bad_streak = 0
                        inner_self._cooldown_until_step = (
                            inner_self.num_timesteps + _cooldown * 1000
                        )
                ...
                def _on_rollout_end(inner_self) -> None:
                    ...
                    # ── Collapse detection ─────────────────────────────────
                    # Skip during LR recovery phase — the model needs time
                    # to stabilise after rollback before we judge it again.
                    if inner_self._recovery_left > 0:
                        return

                    # Skip during timestep-based cooldown (immune period
                    # after rollback to let new episodes populate the buffer).
                    cooldown_until = getattr(inner_self, "_cooldown_until_step", 0)
                    if inner_self.num_timesteps < cooldown_until:
                        return
                    ...
                    # Collapse = reward dropped significantly from peak.
                    # Positive reward: multiplicative threshold.
                    # Negative reward: absolute drop (multiplicative breaks).
                    if inner_self._best_seen > -float("inf"):
                        if inner_self._best_seen > 0:
                            collapsed = mean_r < inner_self._best_seen * _threshold
                        else:
                            drop = inner_self._best_seen - mean_r
                            collapsed = drop > abs(inner_self._best_seen) * (1.0 - _threshold)
                        if collapsed:
                    ...
                    if inner_self._bad_streak >= _patience:
                        should_continue = inner_self._rollback()
                        if not should_continue:
                            inner_self._stop_flag = True

                def _on_step(inner_self) -> bool:
                    if inner_self._orig_learning_starts == 0:
                        inner_self._orig_learning_starts = getattr(
                            inner_self.model, "learning_starts", 0)
                    return not getattr(inner_self, "_stop_flag", False)
...
class SB3Trainer:
    ...
    def train(self):
        ...
        # ── Probe env: extract MJCF metadata (joint names, action dim) ──
        # A short-lived env is created to read ground-truth robot data from
        # the loaded MuJoCo model.  This flows through the spec to the
        # bundle exporter so the manifest matches the actual MJCF.
        try:
            _probe = _make_env(seed_offset=0, use_domain_rand=False)
            self._spec.robot_spec.joint_names = list(
                getattr(_probe, "joint_names", []) or []
            )
            if self._spec.robot_spec.action_dim <= 0:
                self._spec.robot_spec.action_dim = _probe._act_dim
            _probe.close()
            del _probe
        except Exception:
            pass  # static lookup tables will be used as fallback
        ...
        if vec_type == "subproc" and n_envs > 1:
            try:
                ...
                # SubprocVecEnv failed — cap n_envs for DummyVecEnv.
                # DummyVecEnv steps all envs serially on one core, so high
                # n_envs wastes wall-clock time without adding parallelism.
                # Cap at 4 for a reasonable diversity/speed trade-off.
                _dummy_cap = min(n_envs, 4)
                if _dummy_cap < n_envs:
                    env_fns = env_fns[:_dummy_cap]
                    n_envs = _dummy_cap
                if self._log:
                    self._log(
                        f"[SB3] WARNING: SubprocVecEnv failed ({exc}); "
                        f"falling back to DummyVecEnv with n_envs={n_envs} (serial)"
                    )
        ...
        # PPO with small MLP can be slower on GPU due to kernel launch overhead.
        # We log a hint but respect the user's explicit device selection.
        if algo_name == "PPO" and device.startswith("cuda") and self._log:
            self._log(
                "[SB3] Note: PPO with small MLP may be slower on GPU than CPU. "
                "If training is slow, try switching to CPU or use SAC/TD3 for GPU."
            )
        ...
        tau            = float(getattr(algo, "tau", 0.005) or 0.005)
        target_entropy = str(getattr(algo, "target_entropy", "auto") or "auto")
        ...
        model = self._build_or_resume_model(
            ...
            use_per=bool(getattr(algo, "use_per", False)),
            per_alpha=float(getattr(algo, "per_alpha", 0.6)),
            per_beta=float(getattr(algo, "per_beta", 0.4)),
        )
        ...
        # Read back actual gradient_steps from the built model (may have been
        # auto-tuned inside _build_fresh_model for SAC).
        _actual_gs = getattr(model, "gradient_steps", gradient_steps)
        if self._log:
            self._log(f"[SB3] Model built: {algo_name}  seed={seed}  device={device}  "
                      f"gradient_steps={_actual_gs}  n_epochs={n_epochs}")

        # ── Behavioral Cloning pre-training (Phase 1 + Phase 2 setup) ────
        # When ImitationLearningConfig is active and a reference motion is
        # loaded, run supervised BC before RL.  This follows the standard
        # DeepMimic / AMP / DAPG pipeline.
        _bc_blend_callback = None
        _imit_cfg = getattr(self._spec, "imitation_config", None)
        _has_ref = _ref_motion_cfg is not None
        # Auto-enable BC when reference motion is present (unless explicitly
        # disabled by the user setting enabled=False on an existing config)
        _bc_enabled = False
        if _imit_cfg is not None:
            _bc_enabled = bool(getattr(_imit_cfg, "enabled", False))
        elif _has_ref:
            # No explicit config but reference motion is present → auto-enable
            # with sensible defaults so existing graphs get BC for free.
            _bc_enabled = True
            try:
                from src.system.training.training_config import ImitationLearningConfig
                _imit_cfg = ImitationLearningConfig(enabled=True)
            except ImportError:
                _bc_enabled = False

        if _bc_enabled and _has_ref:
            try:
                from src.system.training.behavioral_cloning import (
                    BehavioralCloningTrainer,
                    DemonstrationBuffer,
                )
                # Collect demonstrations from a single env (no VecEnv wrapper)
                demo_env = _make_env(seed_offset=999, use_domain_rand=False)
                if demo_env._ref_frames is not None:
                    _demo_n = int(getattr(_imit_cfg, "demo_num_trajectories", 50) or 50)
                    _demo_noise = float(getattr(_imit_cfg, "demo_noise_std", 0.0) or 0.0)
                    demo_buffer = DemonstrationBuffer.collect(
                        demo_env,
                        num_trajectories=_demo_n,
                        noise_std=_demo_noise,
                        log_fn=self._log,
                    )
                    demo_env.close()
                    ...
                    if demo_buffer.size > 0:
                        ...
                        bc_trainer = BehavioralCloningTrainer(
                            model,
                            demo_buffer,
                            epochs=int(getattr(_imit_cfg, "bc_epochs", 50) or 50),
                            learning_rate=float(getattr(_imit_cfg, "bc_learning_rate", 1e-3) or 1e-3),
                            batch_size=int(getattr(_imit_cfg, "bc_batch_size", 256) or 256),
                            loss_type=str(getattr(_imit_cfg, "bc_loss_type", "mse") or "mse"),
                            log_fn=self._log,
                            cancel_fn=self._cancel,
                        )
                        bc_metrics = bc_trainer.train()
                        if self._metrics:
                            self._metrics({"bc_pretrain": bc_metrics})

                        # Phase 2: build blend callback for BC+RL
                        _blend_steps = int(getattr(_imit_cfg, "bc_blend_steps", 200_000) or 200_000)
                        _blend_coef = float(getattr(_imit_cfg, "bc_blend_coef_start", 0.5) or 0.5)
                        if _blend_steps > 0 and _blend_coef > 0.0:
                            _bc_blend_callback = bc_trainer.build_blend_callback(
                                blend_steps=_blend_steps,
                                coef_start=_blend_coef,
                            )
                            if self._log:
                                self._log(
                                    f"[BC] Phase 2 armed: BC+RL blend for {_blend_steps:,} steps, "
                                    f"λ₀={_blend_coef:.2f} → 0.0 (linear decay)"
                                )
                else:
                    demo_env.close()
                    if self._log:
                        self._log("[BC] Reference motion not loaded in demo env — skipping BC")
            except Exception as _bc_exc:
                if self._log:
                    self._log(f"[BC] BC pre-training failed (non-fatal): {_bc_exc}")
        ...
        callbacks = []
        if self._progress or self._log or self._cancel or self._metrics:
        ...
        if _cg_enabled:
            try:
                guard = SB3CollapseGuard(
                    ...
                    max_rollbacks=int(getattr(algo, "collapse_max_rollbacks", 5) or 5),
        ...
        # ── Phase 2 BC blend callback (must come before model.learn) ─────
        if _bc_blend_callback is not None:
            callbacks.insert(0, _bc_blend_callback.build())
    ...
    def _build_fresh_model(
        ...
        *,
        ...
        if algo_name == "PPO":
            ...
            # Resolve target_entropy: "auto" → SB3 default; numeric string → float
            resolved_target_entropy: Any = "auto"
            te_raw = str(target_entropy).strip().lower()
            if te_raw not in ("auto", ""):
                try:
                    resolved_target_entropy = float(te_raw)
                except ValueError:
                    resolved_target_entropy = "auto"

            # Prioritized Experience Replay (PER) — samples transitions
            # proportional to TD-error, naturally de-prioritizing stale/low-
            # quality experiences without needing explicit episode filtering.
            rb_class  = None
            rb_kwargs: Dict[str, Any] = {}
            if use_per:
                try:
                    from stable_baselines3.common.buffers import PrioritizedReplayBuffer
                    rb_class  = PrioritizedReplayBuffer
                    rb_kwargs = {"alpha": float(per_alpha), "beta": float(per_beta)}
                    if self._log:
                        self._log(
                            f"[SB3] PER enabled — α={per_alpha:.2f}  β={per_beta:.2f}  "
                            "(high-TD-error transitions sampled more frequently)"
                        )
                except ImportError:
                    if self._log:
                        self._log(
                            "[SB3] WARNING: PER requested but PrioritizedReplayBuffer not "
            ...
            sac_kwargs: Dict[str, Any] = dict(
                ...
                target_entropy=resolved_target_entropy,
                verbose=0,
                seed=seed,
                device=device,
                policy_kwargs=policy_kwargs,
            )
            if rb_class is not None:
                sac_kwargs["replay_buffer_class"]  = rb_class
                sac_kwargs["replay_buffer_kwargs"] = rb_kwargs
            return algo_cls_sac(**sac_kwargs)
        else:
            raise ValueError(
