...
class TrainRunThread(QThread):
    ...
    # 4-layer diagnostic metrics: {behavior, stability, action_quality, algorithm}
    ...
    def _run_real(self) -> None:
        ...
        from src.system.training.training_process import (
            MSG_CANCELLED, MSG_ERROR, MSG_EVAL, MSG_FINISHED, MSG_LOG,
            MSG_METRICS, MSG_PROGRESS, run_training_in_process,
        )
        ...
        process = ctx.Process(
            ...
            daemon=False,   # non-daemon so SubprocVecEnv can spawn children
        )
        process.start()

        # Register atexit cleanup so the subprocess is killed if the main
        # process exits unexpectedly (replaces the daemon=True safety net).
        import atexit
        def _cleanup_training_process(proc=process, evt=cancel_event):
            if proc.is_alive():
                evt.set()
                proc.join(timeout=3.0)
                if proc.is_alive():
                    proc.terminate()
        atexit.register(_cleanup_training_process)
        ...
        try:
            while True:
                ...
                if msg_type == MSG_LOG:
                    ...
                    # Use the larger of configured total and actual last step.
                    # SB3 may overshoot total_timesteps by up to one rollout
                    # (n_envs × n_steps steps) before the loop exits, so
                    # self._last_step can be > self._total_timesteps.
                    # Using the configured total here would create a point that
                    # goes *backwards* on the chart relative to the last
                    # progress sample, causing the "U-turn" line artefact.
                    final_step   = max(self._last_step, self._total_timesteps)
                    ...
                    self._record_progress_sample(
                        final_step, final_step, final_reward, final_best, final_ep_len, "Complete"
                    )
                    self._record_run_event(
                        "finished", status="finished", finished=True,
                        bundle_path=_finished_bundle,
                    )
                    self.progress.emit(final_step, final_step, final_reward, final_best, final_ep_len, "Complete")
