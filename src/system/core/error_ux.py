...
Usage::
    ...
    from src.system.core.error_ux import (
        get_operator_text,
        get_error_category,
        classify_run_result,
...
REASON_OPERATOR_TEXT: Dict[str, str] = {
    ...
    # Phase 5 — Transition validation
    "transition_blocked": (
        "Skill sequence contains incompatible transitions with no available "
        "bridging behaviour. Check postcondition/precondition posture and "
        "velocity compatibility between adjacent skills."
    ),
    "transition_velocity_mismatch": (
        "Velocity mismatch between adjacent skills: exit velocity exceeds "
        "the next skill's maximum entry velocity. Insert a deceleration "
        "transition behaviour or adjust skill parameters."
    ),
}
...
REASON_CATEGORY: Dict[str, str] = {
    ...
    # Phase 5 — Transition validation (pre-flight)
    "transition_blocked":             ERROR_CATEGORY_PREFLIGHT,
    "transition_velocity_mismatch":   ERROR_CATEGORY_PREFLIGHT,
...
def format_node_diagnostics(
    ...
    retryable = False
    try:
        from src.system.service.reason_codes import REASON_MAP  # noqa: PLC0415
