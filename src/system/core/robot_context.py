...
Phase 4 additions (additive - all existing callers unchanged):
  - BRAND_ADAPTER_MAP keys corrected to match canonical brand ids
      "boston_dynamics" -> "spot_sdk"   (was "boston_dynamics")
...
from src.system.service.service_registry import ServiceRegistry
from src.system.service.service_router import ServiceRouter, RouteOp
from src.system.service.lifecycle import LifecyclePolicy
from src.system.service.adapters.unitree_sdk2.adapter import UnitreeAdapter
...
class RobotContext:
    ...
    def _create_adapter_for_brand(cls, brand: str, robot_type: str):
        ...
        if brand == "unitree":
            ...
            from src.system.service.adapters.spot_sdk.adapter import SpotAdapter
            return SpotAdapter()
        elif brand == "xiaomi":
            from src.system.service.adapters.cyberdog_sdk.adapter import CyberDogAdapter
    ...
    def _ensure_adapter(cls, brand: str, robot_type: str, force_reinit: bool = False):
        ...
        try:
            ...
            log_warning(f"Adapter connect skipped ({adapter_name}): {e}")
            return None
